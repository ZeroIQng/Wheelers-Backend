...
| `events/payment.events.ts` | `DEPOSIT_RECEIVED`, `NGN_CONVERTING`, `NGN_CONVERTED`, `DRIVER_PAYOUT`, `PENALTY_APPLIED`, `CRYPTO_DEPOSIT_RECEIVED` |
...
│   ├── korapay.route.ts    # POST /webhooks/korapay
│   └── yellowcard.route.ts # POST /webhooks/yellowcard
└── kafka/
...
- On `NGN_CONVERTED` or `CRYPTO_DEPOSIT_RECEIVED`: credits USDT balance (atomic DB transaction)
...
NGN_CONVERTED consumed by:
  ...
  └── notification-worker → sends "Wallet funded +$X.XX" push
```
